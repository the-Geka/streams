...
public class StreamsExercise {
    ...
    public void getAllJobHistoryEntries() {
        ...
        final List<JobHistoryEntry> jobHistoryEntries = employees.stream()
                .flatMap(
                        e -> e.getJobHistory().stream()).collect(toList()
                );
    ...
    public void getSumDuration() {
        ...
        final int sumDurations = employees.stream().mapToInt(
                e -> e.getJobHistory().stream().mapToInt(JobHistoryEntry::getDuration).sum()
        ).sum();
    ...
    public void indexPersonsByEmployer1() {
        ...
        final Map<String, List<PersonEmployer>> index = employees.stream().flatMap(
                e -> e.getJobHistory().stream()
                        .map(JobHistoryEntry::getEmployer)
                        .map((p) -> new PersonEmployer(e.getPerson(), p))
        )
                .collect(Collectors.groupingBy(PersonEmployer::getEmployer, toList()));
    ...
    public void indexPersonsByEmployer2() {
        ...
        final Map<String, List<Person>> index = employees.stream().flatMap(
                e -> e.getJobHistory().stream()
                        .map(JobHistoryEntry::getEmployer)
                        .map((p) -> new PersonEmployer(e.getPerson(), p))
        )
                .collect(
                        Collectors.groupingBy(PersonEmployer::getEmployer,
    ...
    public void getDurationsForEachPersonByPosition() {
        ...
        final List<PersonPositionDuration> personPositionDurations = employees.stream()
                ...
                .flatMap(ppi -> ppi.getDurationByPositionIndex().entrySet().stream()
                        .map(sie -> new PersonPositionDuration(
                                ppi.getPerson(), sie.getKey(), sie.getValue())
                        )
                )
                .collect(toList());
        ...
        final Map<String, PersonPositionDuration> coolestPersonByPosition = employees.stream()
                ...
                .flatMap(ppi -> ppi.getDurationByPositionIndex().entrySet().stream()
                        .map(entry -> new PersonPositionDuration(
                                ppi.getPerson(), entry.getKey(), entry.getValue())
                        )
                )
    ...
    public void getCoolestPersonByPosition2() {
        ...
        final Map<String, Person> coolestPersonByPosition = employees.stream()
                .map(StreamsExercise::getPersonPositionIndex)
                .flatMap(ppi -> ppi.getDurationByPositionIndex().entrySet().stream()
                        .map(sie -> new PersonPositionDuration(ppi.getPerson(), sie.getKey(), sie.getValue())))
                .collect(
                        groupingBy(PersonPositionDuration::getPosition,
                                collectingAndThen(
                                        maxBy(comparing(PersonPositionDuration::getDuration)),
                                        ppi -> ppi.get().getPerson())));
